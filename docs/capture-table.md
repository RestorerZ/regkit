...
Most activities were captured during boot, there are some others such as `Steam.txt`, `TLOU2.txt`, `StartAllBack.txt`, and `Lighshot.txt`, that were captured using [Procmon during use](http://noverse.dev/docs/regkit/guides/procmon/). The records of specific keys are based on `24H2`.
