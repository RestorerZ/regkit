...
bool MainWindow::Impl::HandleMutationCommand(
    int command_id
) {
...
bool MainWindow::Impl::HandleModifyCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kEditChangeType:
    {
      ...
      const std::wstring new_text = value_format::Data(
          result.type,
          result.data.data(),
          static_cast<DWORD>(result.data.size())
      );
...
bool MainWindow::Impl::HandleRenameCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kEditRename:
...
bool MainWindow::Impl::HandleResetDefaultCommand(
    int command_id
) {
  ...
  const std::wstring new_text = value_format::Data(
      choice.type,
      choice.data.data(),
      static_cast<DWORD>(choice.data.size())
  );
  AppendValueHistoryEntry(
      L"Reset value " + display_name,
      old_text,
      new_text,
...
bool MainWindow::Impl::HandleDeleteCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kEditDelete:
    {
      ...
      std::vector<ListRow> selected_rows = SelectedListRows(browse_.values());
      if (focus == browse_.values().hwnd() && selected_rows.size() > 1) {
        const bool all_values = std::all_of(selected_rows.begin(), selected_rows.end(), [](const ListRow& selected) { return selected.kind == rowkind::kValue && !selected.simulated; });
        if (!all_values) {
