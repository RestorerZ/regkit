...
bool MainWindow::Impl::LoadBundledTrace(const std::wstring& label, const trace::Selection* selection_override)
{
...
std::wstring MainWindow::Impl::ResolveBundledDefaultPath(const std::wstring& label) const
{
    ...
    HANDLE find = FindFirstFileW(util::JoinPath(defaults, L"*").c_str(), &data);
    if (find != INVALID_HANDLE_VALUE)
    {
        do
        {
            if (!(data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) || wcscmp(data.cFileName, L".") == 0 ||
...
bool MainWindow::Impl::AddTraceFromFile(const std::wstring& label, const std::wstring& path, const trace::Selection* selection_override, bool prompt_for_selection, bool update_ui)
{
    ...
    if (!session_ptr->selection.select_all && session_ptr->selection.key_paths.empty() &&
        session_ptr->selection.values_by_key.empty())
    {
...
bool MainWindow::Impl::LoadTraceFromFile(const std::wstring& label, const std::wstring& path, const trace::Selection* selection_override)
{
