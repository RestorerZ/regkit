...
inline std::wstring CurrentControlSetSegment()
{
    ...
    ValueEntry entry;
    if (RegistryStore::QueryValue(node, L"Current", &entry) && entry.type == REG_DWORD &&
        entry.data.size() >= sizeof(DWORD))
    {
        DWORD current = 0;
...
inline std::wstring NormalizeTraceKeyPathBasic(const std::wstring& text)
{
    ...
    if (!sid.empty() &&
        (util::EqualsInsensitive(path, current_user) || util::StartsWithInsensitive(path, current_user + L"\\")))
    {
...
inline trace::Normalizers TraceNormalizers()
{
    ...
    normalizers.key = [](const std::wstring& path) { return NormalizeTraceKeyPath(path); };
    normalizers.display = [](const std::wstring& path) { return NormalizeTraceSelectionPath(path); };
    return normalizers;
}
...
inline std::wstring ShortDefaultLabel(const std::wstring& label, const std::wstring& source_path)
{
...
inline std::wstring ResolveOfflineRootName(const std::wstring& path, bool is_dir, const RegistryNode* current_node)
{
    ...
    else
    {
        ...
        if (EqualsInsensitive(base, L"SYSTEM") || EqualsInsensitive(base, L"SOFTWARE") ||
            EqualsInsensitive(base, L"SAM") || EqualsInsensitive(base, L"SECURITY") ||
            EqualsInsensitive(base, L"DEFAULT") || EqualsInsensitive(base, L"COMPONENTS") ||
            EqualsInsensitive(base, L"BCD"))
        {
            return L"HKEY_LOCAL_MACHINE";
        }
    }
