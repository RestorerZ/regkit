...
void MainWindow::Impl::MergeTraceEntries(TraceParseSession* session, const std::vector<KeyValueDialogEntry>& entries, std::unordered_set<std::wstring>* affected_keys)
{
...
void MainWindow::Impl::MergeDefaultEntries(DefaultParseSession* session, const std::vector<KeyValueDialogEntry>& entries, std::unordered_set<std::wstring>* affected_keys)
{
    if (!session || !session->data || entries.empty())
    {
        return;
