...
bool MainWindow::Impl::HandleFavoritesCommand(
    int command_id
) {
  switch (command_id) {
...
bool MainWindow::Impl::HandleNavigateClipboardCommand(
    int command_id
) {
  switch (command_id) {
...
bool MainWindow::Impl::HandleClipboardCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kEditCopyKey:
...
bool MainWindow::Impl::HandleEditToolsCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kEditGoTo:
  ...
  case cmd::kEditFind:
    {
      ...
      sources.reg_files = std::any_of(tabs_.begin(), tabs_.end(), [](const TabEntry& tab) { return tab.kind == TabEntry::Kind::kRegFile && !tab.reg_file_roots.empty(); });
...
bool MainWindow::Impl::HandleChangeHistoryCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kEditPaste:
...
bool MainWindow::Impl::HandleRegistryNavigationCommand(
    int command_id
) {
  switch (command_id) {
  case cmd::kRegistryLocal:
