...
const wchar_t* AnsiCharTable()
{
    static const std::array<wchar_t, 256> table = [] {
        ...
        for (int index = 0; index < 256; ++index)
        {
            ...
            entries[static_cast<size_t>(index)] = (converted && iswprint(wide)) ? wide : L'.';
        }
        return entries;
    }();
    return table.data();
...
std::wstring Preview(std::span<const BYTE> data, int group_bytes, bool unicode)
{
    ...
    const size_t group =
        (group_bytes == 2 || group_bytes == 4 || group_bytes == 8) ? static_cast<size_t>(group_bytes) : 1;
    ...
    for (size_t offset = 0; offset < data.size(); offset += kBytesPerLine)
    {
        ...
        if (unicode)
        {
            for (size_t index = 0; index + 1 < kBytesPerLine; index += 2)
            {
                ...
                const wchar_t value = static_cast<wchar_t>(data[position] | (data[position + 1] << 8));
