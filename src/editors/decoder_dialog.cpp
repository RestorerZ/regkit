...
void FillCombo(
    HWND dialog,
    int control,
    const std::vector<DecoderEntry>& entries,
    int selected
) {
  SendDlgItemMessageW(dialog, control, CB_RESETCONTENT, 0, 0);
...
void RunDecoder(
  ...
  const value_decoder::Decoded decoded =
      value_decoder::Decode(id, bytes.data(), bytes.size());
  if (!decoded.ok) {
...
void ConfigureIdentity(
  ...
  summary.append(L", ").append(std::to_wstring(request.data.size())).append(request.data.size() == 1 ? L" byte" : L" bytes");
...
INT_PTR CALLBACK DialogProc(
  ...
  if (message == WM_INITDIALOG) {
    ...
    state->mono_font = CreateFontW(-12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FF_MODERN, L"Consolas");
    if (state->mono_font) {
...
void ShowValueDecoder(
  ...
  DialogBoxParamW(
      ...
      MAKEINTRESOURCEW(IDD_DECODE_VALUE),
      owner,
      DialogProc,
