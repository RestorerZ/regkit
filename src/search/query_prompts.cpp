...
LRESULT CALLBACK DataTypesDialogProc(
    HWND hwnd,
    UINT msg,
    WPARAM wparam,
    LPARAM lparam
  ...
  case WM_CREATE:
    for (const auto& item : state->items) {
      ...
      const bool checked = state->types.empty() || std::find(state->types.begin(), state->types.end(), item.type) != state->types.end();
...
bool ShowDataTypes(
  ...
  state.types = *types;
  state.rows_per_col = std::max(1, (static_cast<int>(state.items.size()) + kDataTypesColCount - 1) / kDataTypesColCount);
...
LRESULT CALLBACK BrowseDialogProc(
    HWND hwnd,
    UINT msg,
    WPARAM wparam,
    LPARAM lparam
...
bool ShowRegistryKey(
  ...
  state.owner = owner;
  const UINT dpi = win32::DpiForWindow(owner);
  if (!selected_path || !appearance::RunDialogWindow(&state, L"RegKitBrowseKeyDialog", BrowseDialogProc, L"Browse Key", {appearance::metrics::Scaled(420, dpi), appearance::metrics::Scaled(420, dpi)}, WS_SIZEBOX)) {
    return false;
  }
