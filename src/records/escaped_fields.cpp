...
void AppendEscaped(
  ...
  size_t start = 0;
  for (size_t special = text.find_first_of(kSpecial); special != std::wstring_view::npos; special = text.find_first_of(kSpecial, start)) {
...
void AppendUnescaped(
  ...
  size_t start = 0;
  for (size_t slash = text.find(L'\\'); slash != std::wstring_view::npos && slash + 1 < text.size(); slash = text.find(L'\\', start)) {
    const wchar_t next = text[slash + 1];
    const wchar_t decoded = next == L't' ? L'\t' : next == L'r' ? L'\r'
                                               : next == L'n'   ? L'\n'
                                               : next == L'\\'  ? L'\\'
                                                                : 0;
    if (!decoded) {
...
void AppendRecord(
    std::wstring* output,
    std::span<const std::wstring> fields
) {
  AppendFields(output, fields);
}

std::vector<std::wstring> DecodeRecord(
