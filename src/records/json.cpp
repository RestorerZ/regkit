    ...
    case L'u':
      {
        ...
        unsigned second = 0;
        if (end_ - ptr_ < 2 || ptr_[0] != L'\\' || ptr_[1] != L'u' || (ptr_ += 2, !Hex4(&ptr_, end_, &second)) || second < 0xDC00 || second > 0xDFFF) {
          return Fail(L"A string contains a lone surrogate.");
        }
...
void AppendString(
  ...
  for (size_t index = 0; index < text.size(); ++index) {
    ...
    if (character >= 0x20 && character != L'"' && character != L'\\' && character != 0x7F && !high && (character < 0xDC00 || character > 0xDFFF)) {
      continue;
    }
