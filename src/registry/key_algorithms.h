...
inline constexpr SECURITY_INFORMATION kKeySecurityInformation = OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION;

class RegistryKeyHandle {
public:
  RegistryKeyHandle() = default;
  explicit RegistryKeyHandle(
      util::UniqueHKey key
  )
      : key_(std::move(key)) {
  }
...
inline const wchar_t* ValueNameArg(
...
inline void SortNames(
    std::vector<std::wstring>* names
) {
  std::sort(names->begin(), names->end(), [](const std::wstring& left, const std::wstring& right) { return util::CompareInsensitive(left, right) < 0; });
}
...
bool QueryKeyInfo(
  ...
  return key.QueryInfo(&info->subkey_count, nullptr, &info->value_count, nullptr, nullptr, &info->last_write) == ERROR_SUCCESS;
}
...
bool RenameValue(
  ...
  if (ReadValue(key, ValueNameArg(old_name), &type, &data) != ERROR_SUCCESS ||
      ...
      key.SetValue(new_name.c_str(), type, data.empty() ? nullptr : data.data(), static_cast<DWORD>(data.size())) != ERROR_SUCCESS) {
    return false;
  }
...
bool WriteSecurity(
  ...
  return !descriptor.empty() && key.SetSecurity(kKeySecurityInformation, const_cast<BYTE*>(descriptor.data())) == ERROR_SUCCESS;
}
