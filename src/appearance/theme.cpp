...
LRESULT DrawThemedButton(const NMCUSTOMDRAW* draw)
{
    ...
    if (parent)
    {
        HBRUSH parent_brush = reinterpret_cast<HBRUSH>(SendMessageW(
            parent,
            WM_CTLCOLORBTN,
            reinterpret_cast<WPARAM>(hdc),
            reinterpret_cast<LPARAM>(draw->hdr.hwndFrom)
        ));
        if (parent_brush)
        {
    ...
    if (is_checkbox)
    {
        if (disabled)
        {
            state = checked ? CBS_CHECKEDDISABLED : mixed ? CBS_MIXEDDISABLED
                                                          : CBS_UNCHECKEDDISABLED;
        }
        else if (pressed)
        {
            state = checked ? CBS_CHECKEDPRESSED : mixed ? CBS_MIXEDPRESSED
                                                         : CBS_UNCHECKEDPRESSED;
        }
        else if (hot)
        {
            state = checked ? CBS_CHECKEDHOT : mixed ? CBS_MIXEDHOT
                                                     : CBS_UNCHECKEDHOT;
        }
        else
        {
            state = checked ? CBS_CHECKEDNORMAL : mixed ? CBS_MIXEDNORMAL
                                                        : CBS_UNCHECKEDNORMAL;
        }
    }
    else
    ...
    if (focused && show_cues)
    {
        ...
        TEXTMETRICW metrics = {};
        if (DrawTextW(hdc, text, -1, &measured, format | DT_CALCRECT) > 0 && GetTextMetricsW(hdc, &metrics))
        {
...
void PaintGroupBox(HWND hwnd, HDC hdc)
{
    ...
    {
        COLORREF text_color =
            (GetWindowLongPtrW(hwnd, GWL_STYLE) & WS_DISABLED) ? theme.MutedTextColor() : theme.TextColor();
        SetBkMode(hdc, TRANSPARENT);
        SetTextColor(hdc, text_color);
...
void PaintComboBox(HWND hwnd, HDC hdc, ComboBoxThemeState* state)
{
    ...
    bool has_focus = (cb_style == CBS_DROPDOWNLIST && GetFocus() == hwnd) ||
                     (cb_style == CBS_DROPDOWN && info.hwndItem && GetFocus() == info.hwndItem);

    const COLORREF border = !enabled    ? theme.BorderColor()
                            : has_focus ? theme.FocusColor()
    ...
    if (cb_style != CBS_SIMPLE)
    {
        ...
        else
        {
            ...
            HPEN arrow_pen = appearance::CachedPen(arrow_color, 1);
    ...
    if (cb_style == CBS_DROPDOWNLIST)
    {
        ...
        const int item_length =
            index != CB_ERR ? static_cast<int>(SendMessageW(hwnd, CB_GETLBTEXTLEN, index, 0)) : CB_ERR;
        if (item_length > 0)
        {
    ...
    else if (cb_style == CBS_DROPDOWN)
    {
        ...
        HPEN edge_pen = appearance::CachedPen(border, 1);
        HGDIOBJ old_pen = SelectObject(hdc, edge_pen);
        Polyline(hdc, edge, _countof(edge));
        SelectObject(hdc, old_pen);
    }

    HPEN border_pen = appearance::CachedPen(border, 1);
...
LRESULT CALLBACK ComboBoxThemeSubclassProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam, UINT_PTR id, DWORD_PTR ref_data)
{
    ...
    LONG_PTR style = GetWindowLongPtrW(hwnd, GWL_STYLE);
    LONG_PTR cb_style = style & CBS_DROPDOWNLIST;
    bool is_simple = cb_style == CBS_SIMPLE;
    switch (msg)
    {
    case WM_NCDESTROY:
    ...
    case WM_CTLCOLORLISTBOX:
        return reinterpret_cast<LRESULT>(Theme::Current().ControlColor(
            reinterpret_cast<HDC>(wparam),
            reinterpret_cast<HWND>(lparam),
            static_cast<int>(msg - WM_CTLCOLORMSGBOX)
        ));
    case WM_MOUSEMOVE:
        {
            if (state && !state->hot)
    ...
    case WM_NOTIFY:
        {
            ...
            if (hdr && hdr->code == NM_CUSTOMDRAW)
            {
                ...
                if (GetClassNameW(hdr->hwndFrom, class_name, static_cast<int>(_countof(class_name))) &&
                    wcscmp(class_name, WC_BUTTON) == 0)
                {
...
Theme::Theme(const ThemeColors& colors, bool is_dark)
{
    SetColors(colors, is_dark);
}

void Theme::SetColors(const ThemeColors& colors, bool is_dark)
{
    colors_ = colors;
    is_dark_ = is_dark;
    ...
    EnumChildWindows(
        ...
        [](HWND child, LPARAM param) -> BOOL {
            ...
            if (GetClassNameW(child, class_name, static_cast<int>(_countof(class_name))))
            {
                ...
                else if (util::EqualsInsensitive(class_name, WC_EDITW) ||
                         util::StartsWithInsensitive(class_name, L"RICHEDIT"))
                {
                    EnsureSubclass(child, EditShortcutSubclassProc, kEditShortcutSubclassId);
                }
                else if (wcscmp(class_name, WC_BUTTONW) == 0 &&
                         (GetWindowLongPtrW(child, GWL_STYLE) & BS_TYPEMASK) == BS_GROUPBOX)
                {
...
void EnsureSubclass(HWND hwnd, SUBCLASSPROC proc, UINT_PTR id, DWORD_PTR data)
{
