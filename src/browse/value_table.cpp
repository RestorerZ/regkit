...
{
    std::wstring text;
    size_t reserve = row.name.size() + row.type.size() + row.data.size() + row.default_data.size() +
                     row.read_on_boot.size() + row.extra.size() + row.size.size() + row.date.size() +
                     row.details.size() + row.comment.size() + 10;
...
void ValueList::Create(HWND parent, HINSTANCE instance, int control_id)
{
    hwnd_ = CreateWindowExW(
        ...
        WC_LISTVIEWW,
        L"",
        WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | LVS_REPORT | LVS_SHOWSELALWAYS | LVS_OWNERDATA | LVS_EDITLABELS,
        0,
        0,
        100,
        100,
...
int ValueList::AppendRow(ListRow row, bool force_visible)
{
    ...
    if (!visible)
    {
        ...
        visible = filter_cache_.back().find(util::ToLower(filter_text_)) != std::wstring::npos;
    }
    if (!visible)
    {
        return -1;
